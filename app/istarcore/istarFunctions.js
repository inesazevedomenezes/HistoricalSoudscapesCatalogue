...
var istar = function () {
        ...
        var childrenLinks = [];
            ...
            if (connectedLinks) {
                _.forEach(connectedLinks, function (connectedLink) {
                    if (connectedLink.get('target').id === element.id) {
                        childrenLinks.push(connectedLink);
                        childrenNodes.push(istar.graph.getCell(connectedLink.get('source')));
                        }
                });
            }  
    ...
    function _collapseRemove (element) {
        /* jshint validthis: true */
        /* this function is meant to be added to a prototype */

        //var element = this;//stores 'this' in a named variable so that it can be read by the anonymous function            
            var childrenNodes = this.getChildren(element)[0];
                           
            var childrenLinks = this.getChildren(element)[1];
            ...
            if (childrenNodes) {
                ...
                for(i = 0; i < childrenNodes.length; i++){
                    ...
                    var local = childrenNodes[i];
                    
                    var innerChildren = this.getChildren(local)[0];

                    if(innerChildren.length>0){
                        this.collapseRemove(local);  
                    }
                    local.remove();
                }
            }
            ...
            if (childrenLinks) {
                ...
                _.forEach(childrenLinks, function (innerElement) {
                    innerElement.remove();//hide the element's inner links
                });
            }
    }
    
    function _collapseSoundscape (element) {
        /* jshint validthis: true */
        /* this function is meant to be added to a prototype */
        ...
        if (!this.prop('collapsed')) {
                ...
                for(i = 0; i < childrenNodes.length; i++){
                    ...
                    local.attr('./display', 'none');

                    var innerChildren = this.getChildren(local)[0];

                    if(innerChildren.length>0){
                        this.collapseSoundscape(local);  
                    }
                }
                }
                ...
                _.forEach(childrenLinks, function (innerElement) {
                    innerElement.attr('./display', 'none');//hide the element's inner links
                    });
                }
        }
    ...
    function _expandSoundscape (element) {
        /* jshint validthis: true */
        /* this function is meant to be added to a prototype */
        ...
        if (this.prop('collapsed')) {
                ...
                for(i = 0; i < childrenNodes.length; i++){
                    ...
                    local.attr('./display', 'block');

                    var innerChildren = this.getChildren(local)[0];

                    if(innerChildren.length>0){
                        this.expandSoundscape(local);
                    }
                }
                }
                ...
                _.forEach(childrenLinks, function (innerElement) {
                    innerElement.attr('./display', 'block');//hide the element's inner links
                    });
                }
        }
    }
    
    function _toggleSoundscapeCollapse () {
        /* jshint validthis: true */
        /* this function is meant to be added to a prototype */

        if (this.prop('collapsed')) {
            this.expandSoundscape(this);
            this.prop('collapsed', false);

        }
        else {
            this.collapseSoundscape(this);
        ...
        setup: {
            ...
            setupDiagram: function (graph, _paper) {
                ...
                function setupElementLabelFunctions() {
                    ...
                    joint.dia.Element.prototype.toggleSoundscapeCollapse  = _toggleSoundscapeCollapse;
                    joint.dia.Element.prototype.expandSoundscape  = _expandSoundscape;
                    joint.dia.Element.prototype.collapseSoundscape  = _collapseSoundscape;
                    joint.dia.Element.prototype.getChildren  = _getChildren;
                    joint.dia.Element.prototype.collapseRemove  = _collapseRemove;
                }
